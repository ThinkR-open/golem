...
# Creates temp directory in which golems can be created
# and delete it automatically when done
with_dummy_dir <- function(code) {
...
is_properly_populated_golem <- function(path) {
  ...
  if( rstudioapi::isAvailable() ) {
    expected_files <- sort( # demanding on golem name file order might changes 
      c(expected_files, 
  ...
  identical(expected_files, actual_files)
}
