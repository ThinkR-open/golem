...
parse_args <- function(args) {
	...
	while (i <= length(args)) {
		...
		if (identical(arg, "--help") || identical(arg, "-h")) {
			...
			i <- i + 1L
			if (i > length(args)) {
				fail("--functions requires a value")
			}
...
direct_argument_expr <- function(data, call_id, string_id) {
	...
	direct <- chain[vapply(
		chain,
		function(id) identical(parent_of(data, id), call_id),
		logical(1)
	)]
	if (!length(direct)) {
		NA_integer_
...
argument_name <- function(data, call_id, argument_expr_id) {
	...
	children <- children[
		order(children$line1, children$col1, children$id),
		,
		drop = FALSE
	]
...
find_missing_ns_in_file <- function(path, extra_patterns = character()) {
	...
	for (i in seq_len(nrow(strings))) {
		...
		if (
			is.null(call) ||
				!is_candidate_function(call$function_name, extra_patterns)
		) {
			next
		}
...
print_text <- function(results) {
	...
	cat(sprintf(
		"Found %d likely missing ns() call%s:\n",
		nrow(results),
		ifelse(nrow(results) == 1L, "", "s")
	))
