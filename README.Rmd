...
Please format the files with [`air`](https://posit-dev.github.io/air/) before submitting a contribution:

``` sh
air format .
```

The repository also ships a pre-commit hook that runs `air` automatically (see `.pre-commit-config.yaml`).
