...
{golem} has a `.pre-commit-config.yaml` file that defines the hooks. To install them, run:

```
pre-commit install
```

One of the hook is a `lintr`, which will check the code style of the package. It uses the package `{grkstyle}`. You can install it with:

```{r, eval = FALSE}
install.packages("grkstyle")
```

For every commit, the hooks will be run. If one of them fails, the commit will be rejected. You can run the hooks manually with:
